...
def get_corners(frame):
    return [
        (x_all[0, 0, frame], y_all[0, 0, frame]),  # Top-Left
        (x_all[0, -1, frame], y_all[0, -1, frame]),  # Top-Right
        (x_all[-1, 0, frame], y_all[-1, 0, frame]),  # Bottom-Left
        (x_all[-1, -1, frame], y_all[-1, -1, frame])  # Bottom-Right
    ]
...
scat = ax.scatter([c[0] for c in corners], [c[1] for c in corners], s=100, c=['r','g','b','m'], label=corner_labels)

for i, label in enumerate(corner_labels):
    ax.annotate(label, (corners[i][0], corners[i][1]), textcoords="offset points", xytext=(5,5), ha='left', color=scat.get_facecolors()[i])
...
def update(val):
    ...
    # Remove old annotations and add new
    [child.remove() for child in ax.texts]
    for i, label in enumerate(corner_labels):
        ax.annotate(label, (new_corners[i][0], new_corners[i][1]), textcoords="offset points", xytext=(5,5), ha='left', color=scat.get_facecolors()[i])
