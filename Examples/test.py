...
# path = r'C:\Users\konat\Desktop\Thesis\Digtal Image Correlation\muDIC-master\Master_thesis\Examples\set3\aligned\circle_hist_matched'
...
def compute_D1mm_for_path(path, label, real_world_diameter_mm=45.0, n_ely=3, n_elx=3):
    print(f"\nRunning DIC for: {label}")
    ...
    if circles is None:
        raise RuntimeError(f"No circle detected in {label}")
    ...
    mesh = mesher.mesh(
        images,
        Xc1=x_start, Xc2=x_end,
        Yc1=y_start, Yc2=y_end,
        n_ely=n_ely, n_elx=n_elx,
        GUI=False
    )
    ...
    # Extract coordinates
    fields = dic.post.viz.Fields(res, upscale=10)
    x_all = fields.coords()[0, 0, :, :, :]
    y_all = fields.coords()[0, 1, :, :, :]
    ...
    D1_pixels = np.sqrt((top_left_x - bottom_right_x)**2 +
                        (top_left_y - bottom_right_y)**2)

    # Convert to mm
    ref_pixel_diameter = D1_pixels[0]
    scale = real_world_diameter_mm / ref_pixel_diameter
    D1_mm = D1_pixels * scale

    print(f"{label} scale = {scale:.6f} mm/pixel")

    return D1_mm


path_aligned_hist_mached = r"C:\Users\konat\Desktop\Thesis\#Resources\14_HM_tif\aligned\circle_hist_matched"
path_aligned  = r"C:\Users\konat\Desktop\Thesis\#Resources\14_HM_tif\aligned"

D1_orig_mm  = compute_D1mm_for_path(path_aligned_hist_mached, "Original")
D1_align_mm = compute_D1mm_for_path(path_aligned, "Aligned")



plt.figure(figsize=(8,5))

plt.plot(D1_orig_mm,  label="D1 – Aligned and Hist Matched", marker='o')
plt.plot(D1_align_mm, label="D1 – Aligned",  marker='s')

plt.title("Comparison of D1 ( Aligned and Hist Matched vs Aligned) in mm")
plt.xlabel("Frame Number")
plt.ylabel("Diagonal D1 (mm)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
# #-------------------------------------------------------------
# # Circle-based Histogram Matching(only for plot)
...
#                     level=logging.INFO)
# logger = logging.getLogger("circle_hist_match")

# # -------------------------------------------------------------
# # Config: input folder
# # -------------------------------------------------------------
# #path = r'C:\Users\konat\Desktop\Thesis\Digtal Image Correlation\muDIC-master\Master_thesis\Examples\set2\aligned'
# path = r'C:\Users\konat\Desktop\Thesis\#Resources\14_HM_tif\aligned'
...
#     """Return grayscale image, preserving native bit depth."""
#     return cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if img.ndim == 3 else img

# def dtype_max_for(arr):
#     """Return max value for integer dtype."""
#     if np.issubdtype(arr.dtype, np.integer):
#         return np.iinfo(arr.dtype).max
#     return int(np.nanmax(arr))

# def circle_mask(shape, cx, cy, r):
#     """Create boolean mask of a filled circle."""
#     H, W = shape
#     yy, xx = np.ogrid[:H, :W]
#     return (xx - cx)**2 + (yy - cy)**2 <= r**2

# def masked_cdf_uN(img_uN, mask, levels):
#     """CDF of pixel values under a mask (for integer images 0..levels-1)."""
...
#     """
#     Build LUT that maps intensities so that src_cdf(i) ≈ ref_cdf(LUT[i]).
#     """
#     levels = len(src_cdf)
...
# # -------------------------------------------------------------
# # 1) Load reference image & detect circle
# # -------------------------------------------------------------
# ref_image_path = image_files[0]
# logger.info("Using reference image: %s", os.path.basename(ref_image_path))

# ref_img_raw = imread(ref_image_path)
# ref_img_gray = to_gray_native(ref_img_raw)

# # For HoughCircles we normalize to 8-bit
# ref_8bit = cv2.normalize(ref_img_gray, None, 0, 255,
#                          cv2.NORM_MINMAX).astype(np.uint8)
# blurred = cv2.GaussianBlur(ref_8bit, (9, 9), 2)
...
# circles = np.round(circles[0, :]).astype("int")
# x_center, y_center, radius = circles[0]
# logger.info("Detected circle: center=(%d, %d), radius=%d",
#             x_center, y_center, radius)

# # -------------------------------------------------------------
# # 2) Prepare reference CDF inside the detected circle
# # -------------------------------------------------------------
# ref_native = ref_img_gray  # already grayscale native depth
...
# # -------------------------------------------------------------
# # 4) Histogram matching for each image (inside circle) + save
# # -------------------------------------------------------------
...
#     # Ensure same dtype as reference
#     if img.dtype != ref_native.dtype:
#         img = img.astype(ref_native.dtype)

#     # CDF of this image inside same circle region
#     src_cdf = masked_cdf_uN(img, mask_circ, levels)

#     # Build LUT and apply
#     lut = cdf_match_lut(src_cdf, ref_cdf)
#     corrected = lut[img]          # vectorized lookup
#     corrected = corrected.astype(ref_native.dtype)

#     # Save with SAME filename into output folder
#     out_path = os.path.join(out_dir, name)
#     imwrite(out_path, corrected)

# logger.info("Circle-based histogram matched images saved to: %s", out_dir)
