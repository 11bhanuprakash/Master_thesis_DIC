...
# =========================================
# --- 2. ROI DETECTION AND DIC PROCESS ---
# =========================================
def run_dic_pipeline(path, roi_shape, detect_circle=True):
    ...
    if detect_circle and roi_shape == 'C':
        # Try to auto-detect circular ROI
        ...
        if roi_shape == 'C':
            # Circular type but not detected from image (fixed area)
    ...
    mesh = mesher.mesh(images, Xc1=x_start, Xc2=x_end, Yc1=y_start, Yc2=y_end, n_ely=4, n_elx=4, GUI=False)
...
# =========================================
def main():
    print("\n=== Digital Image Correlation Automation ===")
    # base_folder = r'C:\Users\konat\Desktop\Thesis\Digtal Image Correlation\muDIC-master\Master_thesis\Examples\set3'
    base_folder = r'C:\Users\konat\Desktop\Thesis\#Resources\no_grayscale_shifts\no_grayscale_shifts'

    ans_markers = input("Do your images have reference markers? (Y/N): ").strip().upper()
    if ans_markers == 'Y':
        ...
        detect_circle = False  # skip circle detection

    ans_roi = input("Is the ROI circular (C) or rectangular (R)? ").strip().upper()
    if ans_roi not in ['C', 'R']:
        print("Invalid input. Please enter 'C' or 'R'.")
        return

    run_dic_pipeline(data_folder, ans_roi, detect_circle)
